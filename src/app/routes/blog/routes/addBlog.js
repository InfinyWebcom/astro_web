...
class AddBlog extends Component {
    ...
    onSubmit = async () => {
        ...
        if (croppedImage && content) {
            const { values } = this.props.addblogs
            let data = await Axios.axiosHelperFunc('post', 'blog/add', { name: values.name, content: content, blog_image: croppedImage })
...
const mapStateToProps = ({ auth, form }) => {
    ...
    const { addblogs } = form
    return { loader, alertMessage, showMessage, authUser, addblogs, loading }
};
AddBlog = connect(
    mapStateToProps,
...
AddBlog = reduxForm({
    form: 'addblogs',// a unique identifier for this form
