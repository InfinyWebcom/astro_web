...
class EditServices extends Component {
    ...
    componentDidMount = async () => {
        ...
        let data = await Axios.axiosHelperFunc('post', 'service/getServiceDetails', { service_id: this.props.match.params.id })
        if (data.data && data.data.error == false) {
            let { name, description, rate, image_url, commission, commissionPer } = data.data.data
            this.props.initialize({ name: name, description: description, rate: rate, commission: commission, commissionPer: commissionPer })
            this.setState({ croppedImage: `${AppConfig.imageUrl}${image_url}.jpg`, temoImage: image_url, isLoading: false })
        } else {

        }
    }
    submit = () => {
        let { values } = this.props.editservices
        let { name, description, rate, commission, commissionPer } = values
        if (this.state.croppedImage) {
            this.setState({ isLoading: true })
            this.props.addEditServices({ name, description, rate, commission, commissionPer, image_url: this.state.croppedImage.includes('http') ? this.state.temoImage : this.state.croppedImage, url: 'service/editService', service_id: this.props.match.params.id }, this.props.history)
        } else {
...
const mapStateToProps = ({ auth, form }) => {
    ...
    const { editservices } = form
    return { loader, alertMessage, showMessage, authUser, editservices }
};
EditServices = connect(
    mapStateToProps,
    { addEditServices }              // bind account loading action creator
)(EditServices)
EditServices = reduxForm({
    form: 'editservices',// a unique identifier for this form
    enableReinitialize: true,
    keepDirtyOnReinitialize: true,
    initialValues: getFormInitialValues('editservices')(),
    onSubmitSuccess: (result, dispatch) => {
        const newInitialValues = getFormInitialValues('editservices')();
        dispatch(initialize('editservices', newInitialValues));
    },
